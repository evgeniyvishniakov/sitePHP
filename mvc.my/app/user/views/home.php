<?php
include 'include/head.php';
include 'include/header.php';
?>
<section class="main">

	<!-- СЛАЙДЕР   -->

    <div class="main_top">
        <img src="./app/user/views/img/protektor.jpg" alt="slide">
		
    </div>
	
	
	
    <div class="main_catalog">
	
		<!-- НАВИГАЦИЯ НА ГЛАВНОЙ Странице -->
	
        <div class="main_catalog_menu">
            <ul>
                <li><a href="#">Popular</a></li>
                <li><a href="#">New arrivals</a></li>
                <li><a href="#">Best sellers</a></li>
                <li><a href="#">Special offers</a></li>
                <li><a href="#">Coming soon</a></li>
            </ul>
        </div>
		
		<!-- КАТАЛОГ ТОВАРОВ НА ГЛАВНОЙ СТРАНИЦЕ  -->
		
        <div class="main_catalog_products">
            <div class="row">
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <p class="price"><sup>£</sup>29.95</p>
                    <a class="view" href="#"><i class="fa fa-info"></i></a>
                    <a href="#"><img src="./app/user/views/img/photo2.png" alt=""></a>
                    <div class="description">
                        <p class="name">Womens burnt orange casual tee  £29.95</p>
                        <p class="desc">Classic casual t-shirt for women on the move.</p>
                        <p class="comp">100% cotton.</p>
                        <div class="icon">
                            <form action="#" mathod="post">
                                <button type="submit"><i class="fa fa-shopping-cart"></i></button>
                                <button type="submit"><i class="fa fa-heart-o"></i></button>
                                <button type="submit"><i class="fa fa-compress"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>

    </div>
    
	<!-- БЛОК С ЛУКАМИ  -->
	
    <div class="main_lookbook">
        <div class="main_lookbook_man">
            <img src="img/loolbookman.png" alt="">
            <div class="info">
                <h3>Man`s<br><span> Lookbook</span></h3>
                <p>Lorem ipsum dolor sit amet eras facilisis
                consectetur adipiscing elit lor, integer lorem consecteur dignissim laciniqui.
                Elementum metus facilisis ut phasellu.</p>
                <a class="lookbook_btn" href="#">View now</a>
            </div>
        </div>
        <div class="main_lookbook_woman main_lookbook_man">
            <img src="img/loolbookwoman.png" alt="">
            <div class="info">
                <h3>Women`s<br><span> Lookbook</span></h3>
                <p>Lorem ipsum dolor sit amet eras facilisis
                consectetur adipiscing elit lor, integer lorem consecteur dignissim laciniqui.
               Elementum metus facilisis ut phasellu.</p>
                <a class="lookbook_btn" href="#">View now</a>
            </div>
        </div>
    </div>
</section>
<?php
include 'include/footer.php';
?>